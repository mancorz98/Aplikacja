...
class MemProgrammer:
    ...
    def __init__(self, device_name: str = "myDAQ1",fs_acq: float = 10000, N: int = 1e4,r = 4.7, states_limit: tuple = (4, 50), directory= ''):
    ...
    def __set_Ron_state(self, amp: float=1.0, dt: float=0.01, saving=False):
        ...
        if saving:
            dir = os.path.join(self.directory,f"logs_setting_Ron_state_Amp={amp}, dt={dt}")
            self.__loging(t_i,U,I,dir)
    ...
    def __set_Roff_state(self,dt=0.1, amp=-1.5,saving=False):
        ...
        if saving:
            dir = os.path.join(self.directory,f"logs_setting_Roff_state_Amp={amp}, dt={dt}")
            self.__loging(t_i,U,I,dir)
        return q, E
